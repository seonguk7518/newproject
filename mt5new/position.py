        ...
        for position in positions:
            ...
            if a.find_user(position.ticket):
                ...
                try:
                    if conn:
                        with conn.cursor() as cursor:
                            ...
                            sql = "INSERT INTO position_history (fx_id,symbol,sl,tp,profit,price_open,price_current,type,position_id,datetime,updatetime) VALUES " + \
                            f"({df_data['id']},'{position.symbol}','{position.sl}','{position.tp}','{position.profit}','{position.price_open}','{position.price_current}','{position.type}',{position.ticket},'{datetimeobj}','{aaa1}')"
                            cursor.execute(sql)
                ...
                with conn.cursor() as cur:
                    ...
                    sql = """UPDATE position_history set sl=%s,tp=%s,profit=%s,price_open=%s,price_current=%s,updatetime=%s where position_id=%s  """
