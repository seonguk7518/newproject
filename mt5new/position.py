...
for i in df_user_dict.iterrows():
    ...
    if positions==None:
        ...
        for position in positions:
            ...
            cad=(Decimal(position.price_open)*(Decimal(position.volume)*Decimal('100000')))/500
            cad=round(cad,2)
            ...
            if a.find_user(position.ticket):
                ...
                try:
                    if conn:
                        with conn.cursor() as cursor:
                            ...
                            sql = "INSERT INTO position_history (fx_id,symbol,sl,tp,profit,price_open,price_current,type,volume,cad,position_id,datetime,updatetime) VALUES " + \
                            f"({df_data['id']},'{position.symbol}','{position.sl}','{position.tp}','{position.profit}','{position.price_open}','{position.price_current}','{position.type}','{position.volume}','{cad}',{position.ticket},'{datetimeobj}','{aaa1}')"
                            cursor.execute(sql)
                ...
                with conn.cursor() as cur:
                    ...
                    sql = """UPDATE position_history set sl=%s,tp=%s,profit=%s,price_open=%s,price_current=%s,volume=%s,cad=%s,updatetime=%s where position_id=%s  """
