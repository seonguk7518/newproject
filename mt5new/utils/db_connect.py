        ...
        if self.new_find_user(user_num)==False: 
            ...
            if conn:
                with conn.cursor() as cursor:
                    ...
                    sql1 = f"select count(symbol)from Main  as u inner join position_history  as b on u.fx_id =b.fx_id  WHERE u.retri_id ='{user_num}' and date_format( updatetime  , '%Y-%m-%d %H:%i:%s') >='{now_time}' order by `updatetime`  asc limit {new_size},{size};"
                    cursor.execute(sql)
                    result=cursor.fetchall()
                    result=pd.DataFrame(result)
                    print("sdsds",result)
