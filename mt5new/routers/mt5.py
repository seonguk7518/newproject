...
@router.get('/mt5-user-position-count', summary='Api USER MT5 POSITION COUNT', tags=['MT5 USER'])
async def api_select(fx_id:int):
    ...
    try:    
        mysql.user_position_count(fx_id)
...
@router.get('/mt5-manager-individual-position-count', summary='Api USER MT5 POSITION INDIVIDUAL COUNT', tags=['MT5 MANAGER '])
async def api_select(retri_id:str):
    ...
    try:    
        mysql.manage_position_count(retri_id)
...
@router.get('/mt5-manager-deposit', summary='Api MT5 DEPOSIT ', tags=['MT5 MANAGER '])
async def api_select(retri_id:str):

    mysql=MySQLAdapter()
    
    try:    
        mysql.manager_deposit_list(retri_id)
...
async def get_deposit_info(
    ...
    try:
        ...
        if year:
            main_query = main_query.where(extract('year', main_table.c.datetime) == year)
        if month:
            main_query = main_query.where(extract('month', main_table.c.datetime) == month)
        ...
        if main_result:
            ...
            return_dict_data['message'] = 'FX ID is incorrect or no data found for the specified date'
        ...
        if year:
            monthly_money_query = monthly_money_query.where(extract('year', monthly_money_table.c.datetime) == year)
        if month:
            monthly_money_query = monthly_money_query.where(extract('month', monthly_money_table.c.datetime) == month)
        ...
        balance_query = select([balance_table.c.deposit, balance_table.c.datetime, balance_table.c.check]).where(
            (balance_table.c.fx_id == fx_id)
        )
        
        if year:
            balance_query = balance_query.where(extract('year', balance_table.c.datetime) == year)
        if month:
            balance_query = balance_query.where(extract('month', balance_table.c.datetime) == month)

        offset = (page - 1) * size
        balance_query = balance_query.order_by(balance_table.c.datetime).limit(size).offset(offset)
        ...
        if year:
            total_count_query = total_count_query.where(extract('year', balance_table.c.datetime) == year)
        if month:
            total_count_query = total_count_query.where(extract('month', balance_table.c.datetime) == month)
