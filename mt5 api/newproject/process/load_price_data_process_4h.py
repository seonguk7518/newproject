...
def generate_price_dict(usdt_keys, datetime ) : 
    ...
    for usdt_key in usdt_keys :
        try : 
            coin_data = load_coin_data(usdt_key,'4h').iloc[-2]
...
def insert_data_to_db(price_data_list, conn) : 
    with conn.cursor() as cur : 
        for price_data in price_data_list : 
            ...
            sql = f"Insert into ticker_price_score_4h (datetime,ticker,open,high,low,close,volume) values ({values_String})"
            cur.execute(sql)
        conn.commit()

date= (datetime.datetime.now(timezone('Asia/Seoul'))-datetime.timedelta(hours=4)).strftime("%Y%m%d%H")
