...
def load_coin_data(symbol, time_m) :   #타입 변환
    dt_utc = datetime.datetime.now() + datetime.timedelta(hours=-48)  #KST -> UTC
...
def generate_price_dict(usdt_keys, datetime ) : 
    data_list = []
    for usdt_key in usdt_keys :
        try : 
            coin_data = load_coin_data(usdt_key,'30m').iloc[-2]
...
def insert_data_to_db(price_data_list, conn) : 
    with conn.cursor() as cur : 
        for price_data in price_data_list : 
            ...
            sql = f"Insert into ticker_price_score_30m (datetime,ticker,open,high,low,close,volume) values ({values_String})"
            cur.execute(sql)
        conn.commit()

date= (datetime.datetime.now(timezone('Asia/Seoul'))-datetime.timedelta(hours=4)).strftime("%Y%m%d%H%M")

usdt_key_list = filter_usdt_key(coins_trd)
price_data_dict = generate_price_dict(usdt_key_list,date)
